...
public class AcademyMemberArrayMain {
	...
	public static void main(String[] args) {
		...
		*/
		...
		for (int i = 0; i < members.length; i++) {
			if(members[i] instanceof AcademyStudent) {
				AcademyStudent ads =(AcademyStudent)members[i];
				if(ads.getBan().equals("자바")) {
					ads.print();
				}
			}
		}
		...
		for (int i = 0; i < members.length; i++) {
			if(members[i] instanceof AcademyGangsa) {
				AcademyGangsa ads =(AcademyGangsa)members[i];
				if(ads.getSubject().equals("자바")) {
					ads.print();
				}
			}
		}
		...
		for (int i = 0; i < members.length; i++) {
			if(members[i] instanceof AcademyStaff) {
				AcademyStaff ads =(AcademyStaff)members[i];
				if(ads.getDepart().equals("영업")) {
					ads.print();
				}
			}
		}

...
--로그인한멤버(guard1)의 카트에 제품존재하는 제품의수(제품존재여부판단)
select count(*) cnt from cart; where userid='guard' and p_no=1;

--로그인한멤버(guard1)의 카트에담기(수정)
update cart set cart_qty = cart_qty+1 where userid='guard'and p_no=1;
...
join product p
on oi.p_no = p.p_no
where userid='guard1'and o.o_no=2;
